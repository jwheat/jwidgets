...
//console.log(widgetItems[x]);
   ...
   //var num_of_links = widgetItems[x].settings['number_of_links'];
    ...
    //if (num_of_links === undefined) {
        ...
        // when settings_count defaults to 1 - editing a widget adds an extra link at the bottom
        // when settings_count defaults to 0 - the first 2 link sets are param_1 - so you loose your first link
        var setting_count = 1;
        for (j = 1; j < 50; j++) {
            var local_param_url = widgetItems[x].settings['param_'+j+'_url'];
            var local_param_text= widgetItems[x].settings['param_'+j+'_link_text'];
...
//    for (i = 2; i < parseInt(num_of_links); i++) {
        add_rows(i);
...
// function to remove a link (2 rows)
    ...
    function remove_row(rowid) {
        ...
        // remove input fields first
        alert ('#param_'+rowid+'_url');

        var title_element = document.getElementById("param_"+rowid+"_url");
        title_element.parentNode.removeChild(title_element);

        var link_element = document.getElementById("param_"+rowid+"_link_text");
