<?php
    ...
    if (!empty($settings)) {
        ...
        foreach ($settings as $value) {
            ...
            if ($value->name == 'nav_widget_title_url') {
                $nav_widget_title_url = $value->value;
            }
        }
    }
    ...
    if ($nav_widget_title != '' && $nav_widget_title_url == '') {
?>
    <h2><?php print encodeHtml($nav_widget_title); ?></h2>
<?php
    }
    else if ($nav_widget_title != '' && $nav_widget_title_url != '') {
?>    	
    <h2><a href="<?php if (strpos($nav_widget_title_url, 'http://') !== 0 && strpos($nav_widget_title_url, 'https://') !== 0) { print 'http://'; } print encodeHtml($nav_widget_title_url); ?>"><?php print encodeHtml($nav_widget_title); ?></a></h2>	
<?php    	
    }
...
?>
        <li>
            <a href="<?php if (strpos($widgetLink[1], 'http://') !== 0 && strpos($widgetLink[1], 'https://') !== 0) { print 'http://'; } print encodeHtml($widgetLink[1]); ?>"><?php print encodeHtml($widgetLink[0]); ?></a>
        </li>
<?php
        }
?>
