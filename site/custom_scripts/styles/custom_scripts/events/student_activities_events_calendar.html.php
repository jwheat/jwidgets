...
<meta name="dcterms.description"
      content="<?php print encodeHtml(METADATA_GENERIC_NAME); ?>'s index of events organised within the following categories<?php foreach ($dirTree as $parent) {
          print ', ' . encodeHtml($parent->name);
      } ?>">
<meta name="dcterms.subject" content="<?php foreach ($dirTree as $parent) {
    print encodeHtml($parent->name) . ', ';
}
...
<meta name="keywords" content="<?php if(is_array($dirTree)) foreach ($dirTree as $parent) {
    print encodeHtml($parent->name) . ', ';
}
...
<?php
require_once 'student_activities_closing.php';
?>
